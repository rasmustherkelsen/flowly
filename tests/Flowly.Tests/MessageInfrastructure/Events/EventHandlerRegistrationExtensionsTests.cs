...
public class EventHandlerRegistrationExtensionsTests
{
    public class AddEventHandler
    {
        ...
        public void WithEventNameAttribute_UsesAttributeValue()
        {
            ...
            Assert.Equal("custom-event", manifest.Events[0].TopicOrExchangeName);
        }
    ...
    private static (IFlowlyBuilder Builder, ProviderQueueManifest Manifest) CreateBuilder(string providerName)
    {
        ...
        var manifest = new ProviderQueueManifest(providerName, isPrimary: true, "AzureServiceBus");
    ...
    private class OrderNotificationHandler : Flowly.MessageInfrastructure.Events.EventHandlerBase<OrderPlaced>
    {
        public override Task Handle(IEventContext<OrderPlaced> eventContext, CancellationToken cancellationToken)
            => Task.CompletedTask;
    }

    private class AuditLogHandler : Flowly.MessageInfrastructure.Events.EventHandlerBase<OrderPlaced>
    {
        public override Task Handle(IEventContext<OrderPlaced> eventContext, CancellationToken cancellationToken)
            => Task.CompletedTask;
    }

    [RetryPolicyAttribute(3, 10)]
    private class RetryingOrderHandler : Flowly.MessageInfrastructure.Events.EventHandlerBase<OrderPlaced>
    {
        public override Task Handle(IEventContext<OrderPlaced> eventContext, CancellationToken cancellationToken)
            => Task.CompletedTask;
    }

    private class NamedEventHandler : Flowly.MessageInfrastructure.Events.EventHandlerBase<NamedEvent>
    {
