...
namespace Flowly.Jobs.SQLite.Tests.Registration;

public class SQLiteJobStateTrackerRegistrationExtensionTests
{
    public class AddSQLiteJobStateTracking
    {
        [Fact]
        public void WithConnectionStringName_ResolvesConnectionStringFromConfiguration()
        {
            var configuration = new ConfigurationBuilder()
                ...
                {
                    ["ConnectionStrings:JobsDb"] = "Data Source=cfg-jobs.db"
                })
                .Build();
            var builder = CreateFullBuilder(configuration);

            builder.AddSQLiteJobStateTracking("JobsDb");

            Assert.Equal("Data Source=cfg-jobs.db", ResolveConnectionString(builder.Services));
        }
        ...
        public void WithLiteralConnectionString_UsesValueDirectly()
        {
            ...
            builder.AddSQLiteJobStateTracking("Data Source=direct-jobs.db");

            Assert.Equal("Data Source=direct-jobs.db", ResolveConnectionString(builder.Services));
        }
    }
    ...
    public class AddJobStateTrackingClient
    {
        ...
        public void WithLiteralConnectionString_UsesValueDirectly()
        {
            ...
            builder.AddJobStateTrackingClient("Data Source=direct-jobs.db");
