...
public class RabbitMqRetryTopologyValidatorTests
{
    public class Validate
    {
        ...
        [Fact]
        public async Task WhenRetryQueueMissing_ErrorMessageIncludesRoutingKeyInstructions()
        {
            var pool = new FakeConnectionPool(new FailingChannel());
            var validator = new RabbitMqRetryTopologyValidator("rabbitmq", pool);
            ...
            Assert.Contains("order-created", exception.Message);
        }

        [Fact]
        public async Task WhenAllQueuesHaveRetryQueue_ValidatesAll()
        {
            var pool = new FakeConnectionPool(new SucceedingChannel(), new SucceedingChannel(), new SucceedingChannel());
            var validator = new RabbitMqRetryTopologyValidator("rabbitmq", pool);
    ...
    public class ProviderName
    {
        ...
        {
            var pool = new FakeConnectionPool();
            var validator = new RabbitMqRetryTopologyValidator("my-rabbit", pool);
