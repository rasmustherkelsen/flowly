...
internal static class RecurringJobHandlerOptionsResolver
{
    public static ResolvedRecurringJobHandlerOptions Resolve<TRecurringJobHandler>() where TRecurringJobHandler : RecurringJobHandler
    {
        ...
        var resolvedCronExpression = string.IsNullOrWhiteSpace(options.CronExpression)
            ? throw new InvalidOperationException($"Recurring job handler '{handlerType.Name}' must define a cron expression via {nameof(RecurringJobAttribute)} or {nameof(RecurringJobHandler.Configure)}.")
    ...
    private static void ApplyAttributes(Type handlerType, RecurringJobHandlerOptions options)
    {
        ...
        if (string.IsNullOrWhiteSpace(options.JobDescription))
        {
            ...
            if (descriptionAttribute != null) options.JobDescription = descriptionAttribute.Description;
        }
    }

    private static void ApplyConfigure(Type handlerType, RecurringJobHandlerOptions options)
    {
        var configureMethod = handlerType.GetMethod(nameof(RecurringJobHandler.Configure), [typeof(RecurringJobHandlerOptions)]);

        if (configureMethod is null || configureMethod.DeclaringType == typeof(RecurringJobHandler)) return;

        if (CreateHandlerInstanceForConfigure(handlerType) is not { } handlerInstance) return;
    ...
    private static string DeriveDefaultDescription(Type handlerType)
    {
        ...
        if (name.EndsWith("RecurringJob", StringComparison.OrdinalIgnoreCase)) return name[..^"RecurringJob".Length];

        if (name.EndsWith("Handler", StringComparison.OrdinalIgnoreCase)) return name[..^"Handler".Length];
