...
Rules:
- Always inherit **both** `FlowlyDesignTimeFactory` and `IFlowlyConfiguration`.
- `FlowlyDesignTimeFactory` enables the `flowly` CLI tool and the Aspire integration to discover queue topology without running the app.
...
**Auto-discovery** (finds the first `IFlowlyConfiguration` in the assembly — use only in single-configuration projects):
```csharp
builder.AddFlowly();
```
...
- [ ] `FlowlyConfiguration.cs` created (inherits both `FlowlyDesignTimeFactory` and `IFlowlyConfiguration`)
