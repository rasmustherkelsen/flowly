...
A persistent store backend is also **required**. Check whether job state tracking is already registered in the project's `IFlowlyConfiguration`. If it is, no changes are needed. If it is not, choose a backend:
...
Find the `IFlowlyConfiguration` implementation in the project and add:
...
- [ ] Registered with `AddRecurringJob<TH>()` in `IFlowlyConfiguration`
