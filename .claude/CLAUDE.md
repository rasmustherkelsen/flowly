...
| `MessageStreamHandler<T>` | Append-only, replayable message stream — **RabbitMQ and InMemory only** (requires `IStreamCapableMessageBusClient`; throws at startup on Azure Service Bus). InMemory backs it with an in-process log (`InMemoryStreamLog`), not a broker — no cross-process sharing, no cross-restart persistence, and this is not changed by registering a checkpoint (throws instead). RabbitMQ streams gain opt-in restart-survival via `MessageStreamCheckpoint<T>` — see "Position Persistence" below. Optionally partitioned via `[StreamPartitions(count)]` on the contract — see "Partitioned Streams" below. | Yes — in-process retry on the same batch, no requeue (would corrupt the replayable log); halts consumption entirely (does not advance the offset or skip) once exhausted; for a partitioned stream, halting only stops that partition | No | `.AddMessageStreamHandler<T, TH>()` (receiver) / `.AddMessageRecorder<T>()` (sender) |
...
- `IMessageRecorder.Record<T>(msg, ct, partitionKey)` — records onto an append-only, replayable stream (requires `.AddMessageRecorder<T>()`; RabbitMQ or InMemory). `partitionKey` (optional, trailing) selects a partition on a stream partitioned via `[StreamPartitions]` — same key always routes to the same partition; omit for round-robin
...
### Position Persistence (`MessageStreamHandler<T>` only)

Opt-in, transport-agnostic, user-implemented checkpointing that restores restart-survival — by default `StartPosition` is re-evaluated fresh on every process boot. Implement `MessageStreamCheckpoint<TMessage>` (`InitializeCheckpoint`, `GetStreamPosition`, `SaveStreamPosition`) and register it via plain DI (`services.AddSingleton<MessageStreamCheckpoint<TMessage>, TImpl>()`) — Flowly feature-detects it automatically, same pattern as Jobs/DeadLetters on the Dashboard. Once registered, `MessageStreamHandlerOptions.StartPosition` becomes a bootstrap value used only on the first run; afterwards the stored position (keyed by `ConsumerName` — defaults to the handler type name, overridable — and `Partition`, `null` for non-partitioned streams) takes over. RabbitMQ only — registering a checkpoint against an InMemory-backed stream throws `InvalidOperationException` at registration time, since the underlying log itself has no cross-restart persistence to checkpoint against. Flowly does not coordinate exclusive access to a checkpoint store across processes; running more than one instance of the same non-partitioned handler against a shared store is unsupported and documented, not enforced.

### Partitioned Streams (`MessageStreamHandler<T>` only)

Divide a stream into `N` independent, ordered sub-logs with `[StreamPartitions(count)]` on the message contract — additive, a non-partitioned stream is unaffected. Requires the resolved provider's client to implement `IPartitionedStreamCapableMessageBusClient`; RabbitMQ (Super Streams, broker-coordinated Single Active Consumer rebalancing) and InMemory (parity/local-dev only — single-process, so every partition is owned immediately and permanently, no cross-instance scale-out benefit) both implement it. Flowly does not build its own cross-instance partition-assignment protocol — that's entirely the transport's job; Flowly runs one independent batch-accumulate-and-retry loop per partition currently owned by this process, so a halted (retries-exhausted) partition only stops that partition. `IMessageStreamContext<T>.Partition` tells the handler which partition a batch came from. Position persistence composes unchanged — `MessageStreamCheckpoint<TMessage>` is queried per partition via the same `(ConsumerName, Partition)` key, resolved through the transport's own rebalancing callback when a partition is newly assigned. Producer-side routing is `IMessageRecorder.Record(msg, ct, partitionKey)` — same key always lands in the same partition; omit for round-robin.
