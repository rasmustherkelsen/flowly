...
| `[StreamPartitions(count)]` | Divides the stream into `count` independent, ordered sub-logs (`count` must be at least 1 — the constructor throws `ArgumentOutOfRangeException` otherwise). RabbitMQ implements it via Super Streams with broker-coordinated Single Active Consumer rebalancing (genuine cross-instance scale-out); InMemory implements it for dev/test parity only — single-process, so no real scale-out benefit. Requires `IPartitionedStreamCapableMessageBusClient` | Message contract consumed via `MessageStreamHandler<T>` or recorded via `IMessageRecorder` (RabbitMQ or InMemory) | [Partitioned streams](../README.md#partitioned-streams) |
