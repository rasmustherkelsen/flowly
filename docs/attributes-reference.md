...
| `[StreamRetention(maxAgeSeconds, maxLengthBytes)]` | Sets the RabbitMQ stream's `x-max-age`/`x-max-length-bytes` retention limits; omitting both means the stream grows unbounded | Message contract consumed via `MessageStreamHandler<T>` or recorded via `IMessageRecorder` (RabbitMQ only) | [Message Streaming](../README.md#message-streaming) |
