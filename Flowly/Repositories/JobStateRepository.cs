...
using Job = Flowly.DatabaseModel.JobStateDatabase.Job;
using JobStateDatabase_Job = Flowly.DatabaseModel.JobStateDatabase.Job;
...
internal class JobStateRepository(IDbContextFactory<JobStateDataContext> jobStateDataContextFactory) : IJobStateRepository
{
    ...
    public async Task CreateJobState(CreateJobState createJobState)
    {
        ...
        var job = new JobStateDatabase_Job
        {
    ...
    private async Task<JobStateDatabase_Job> ResilientGetJob(JobStateDataContext context, Guid jobId)
    {
        int retryCount = 0;
