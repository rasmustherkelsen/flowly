...
1. Generate the emulator queue config and start the emulator and SQL Server:
   ...
   docker compose up -d
   ```
   `GenerateSbConfig.ps1` builds and installs the `flowly` CLI, introspects `Sender` and `Receiver` to discover message contracts, and writes `sbconfig.json` — the queue configuration file the ASB emulator requires. Re-run it whenever message contracts change.
