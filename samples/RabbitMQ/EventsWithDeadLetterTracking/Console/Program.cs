...
static void PrintTable(List<IDeadLetter> deadLetters)
{
    ...
    const int colTopic = 32;
    const int colSubscription = 28;
    const int colReason = 38;
    ...
    Console.WriteLine(
        ...
        $"{"Topic / Exchange".PadRight(colTopic)}" +
        $"{"Subscription".PadRight(colSubscription)}" +
        $"{"Reason".PadRight(colReason)}" +
        $"{"Dead-lettered at".PadRight(colDate)}");

    Console.WriteLine(new string('-', colNum + colStatus + colTopic + colSubscription + colReason + colDate));
    ...
    for (var i = 0; i < deadLetters.Count; i++)
    {
        ...
        Console.WriteLine(
            ...
            $"{Truncate(dl.QueueName, colTopic).PadRight(colTopic)}" +
            $"{Truncate(dl.SubscriptionName ?? "-", colSubscription).PadRight(colSubscription)}" +
